        ...
        <div className='space-y-4'>
          {data.map((experience, index) => (
            <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
              <div className='flex justify-between items-center'><h4>Experience #{index + 1}</h4>
              ...
              <div className='grid md:grid-cols-2 gap-3'>
                ...
                <input value={experience.position || ""} onChange={(e) => updateExperience(index, "positon", e.target.value)} type="text" placeholder='job title' className='px-3 py-2 text-sm rounded-lg' />
                ...
                <input value={experience.end_date || ""} onChange={(e) => updateExperience(index, "end_date", e.target.value)} disabled={experience.is_current} type="month" className='px-3 py-2 text-sm rounded-lg disabled:bg-gray-100' />
              </div>
              <label>
                <input type="checkbox" checked={experience.is_current || false} onChange={(e) => { updateExperience(index, "is_current", e.target.checked ? true : false); }} className='rounded border-gray-300 text0blue-600 focus:ring-blue-500' />
